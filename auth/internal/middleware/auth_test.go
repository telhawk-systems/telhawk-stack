...
func TestRequireAuth_Success(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Verify roles in context
		ctxRoles := r.Context().Value(RolesKey)
		if ctxRoles == nil {
...
func TestRequireRole_Success(t *testing.T) {
	...
	httpReq := httptest.NewRequest("GET", "/admin", nil)
	httpReq.Header.Set("Authorization", "Bearer "+loginResp.AccessToken)

	rr := httptest.NewRecorder()
	mw.RequireRole("admin")(handler).ServeHTTP(rr, httpReq)
...
func TestRequireRole_MissingRole(t *testing.T) {
	...
	// Create user with only viewer role
	_, token := createTestUser(t, svc)
	...
	rr := httptest.NewRecorder()
	mw.RequireRole("admin")(handler).ServeHTTP(rr, req)

	if handlerCalled {
		t.Error("Handler should not be called when user lacks required role")
	}
...
func TestRequireRole_NoAuthHeader(t *testing.T) {
	...
	// RequireRole calls RequireAuth first, so we expect 401
	if status := rr.Code; status != http.StatusUnauthorized {
		t.Errorf("Expected status 401, got %d", status)
	}
}
...
func TestRequireRole_MultipleRoles(t *testing.T) {
	...
	tests := []struct {
		...
		{"HasViewer", "viewer", http.StatusOK},
		{"HasAnalyst", "analyst", http.StatusOK},
		{"LacksAdmin", "admin", http.StatusForbidden},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rr := httptest.NewRecorder()
			mw.RequireRole(tt.requiredRole)(handler).ServeHTTP(rr, req)

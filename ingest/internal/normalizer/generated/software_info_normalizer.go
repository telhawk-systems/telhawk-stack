...
// SoftwareInfoNormalizer normalizes software_info events to OCSF format
type SoftwareInfoNormalizer struct{}

// NewSoftwareInfoNormalizer creates a new normalizer
func NewSoftwareInfoNormalizer() *SoftwareInfoNormalizer {
	return &SoftwareInfoNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *SoftwareInfoNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *SoftwareInfoNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewSoftwareInfo(activityID)

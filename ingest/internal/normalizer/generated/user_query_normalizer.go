...
// UserQueryNormalizer normalizes user_query events to OCSF format
type UserQueryNormalizer struct{}

// NewUserQueryNormalizer creates a new normalizer
func NewUserQueryNormalizer() *UserQueryNormalizer {
	return &UserQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *UserQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *UserQueryNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewUserQuery(activityID)

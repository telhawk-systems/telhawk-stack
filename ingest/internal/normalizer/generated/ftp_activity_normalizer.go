...
// FtpActivityNormalizer normalizes ftp_activity events to OCSF format
type FtpActivityNormalizer struct{}

// NewFtpActivityNormalizer creates a new normalizer
func NewFtpActivityNormalizer() *FtpActivityNormalizer {
	return &FtpActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *FtpActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *FtpActivityNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := network.NewFtpActivity(activityID)

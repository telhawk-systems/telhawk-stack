...
// PeripheralDeviceQueryNormalizer normalizes peripheral_device_query events to OCSF format
type PeripheralDeviceQueryNormalizer struct{}

// NewPeripheralDeviceQueryNormalizer creates a new normalizer
func NewPeripheralDeviceQueryNormalizer() *PeripheralDeviceQueryNormalizer {
	return &PeripheralDeviceQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *PeripheralDeviceQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *PeripheralDeviceQueryNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewPeripheralDeviceQuery(activityID)

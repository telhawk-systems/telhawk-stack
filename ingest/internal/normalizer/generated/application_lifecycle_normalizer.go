...
// ApplicationLifecycleNormalizer normalizes application_lifecycle events to OCSF format
type ApplicationLifecycleNormalizer struct{}

// NewApplicationLifecycleNormalizer creates a new normalizer
func NewApplicationLifecycleNormalizer() *ApplicationLifecycleNormalizer {
	return &ApplicationLifecycleNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *ApplicationLifecycleNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *ApplicationLifecycleNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := application.NewApplicationLifecycle(activityID)

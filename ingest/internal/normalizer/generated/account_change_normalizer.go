...
// AccountChangeNormalizer normalizes account_change events to OCSF format
type AccountChangeNormalizer struct{}

// NewAccountChangeNormalizer creates a new normalizer
func NewAccountChangeNormalizer() *AccountChangeNormalizer {
	return &AccountChangeNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *AccountChangeNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "account_change") || 
	       strings.Contains(st, "account")
}

// Normalize converts raw event to OCSF account_change
func (n *AccountChangeNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := iam.NewAccountChange(activityID)

...
// ScriptActivityNormalizer normalizes script_activity events to OCSF format
type ScriptActivityNormalizer struct{}

// NewScriptActivityNormalizer creates a new normalizer
func NewScriptActivityNormalizer() *ScriptActivityNormalizer {
	return &ScriptActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *ScriptActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *ScriptActivityNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := system.NewScriptActivity(activityID)

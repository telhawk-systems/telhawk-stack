...
import (
	...
	"github.com/telhawk-systems/telhawk-stack/common/ocsf/events/iam"
	"github.com/telhawk-systems/telhawk-stack/common/ocsf/objects"
)

// AuthorizeSessionNormalizer normalizes authorize_session events to OCSF format
type AuthorizeSessionNormalizer struct{}

// NewAuthorizeSessionNormalizer creates a new normalizer
func NewAuthorizeSessionNormalizer() *AuthorizeSessionNormalizer {
	return &AuthorizeSessionNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *AuthorizeSessionNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "authorize_session") || 
	       strings.Contains(st, "authorize")
}

// Normalize converts raw event to OCSF authorize_session
func (n *AuthorizeSessionNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := iam.NewAuthorizeSession(activityID)

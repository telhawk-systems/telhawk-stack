...
// ApplicationErrorNormalizer normalizes application_error events to OCSF format
type ApplicationErrorNormalizer struct{}

// NewApplicationErrorNormalizer creates a new normalizer
func NewApplicationErrorNormalizer() *ApplicationErrorNormalizer {
	return &ApplicationErrorNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *ApplicationErrorNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *ApplicationErrorNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := application.NewApplicationError(activityID)

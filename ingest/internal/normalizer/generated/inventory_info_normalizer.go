...
// InventoryInfoNormalizer normalizes inventory_info events to OCSF format
type InventoryInfoNormalizer struct{}

// NewInventoryInfoNormalizer creates a new normalizer
func NewInventoryInfoNormalizer() *InventoryInfoNormalizer {
	return &InventoryInfoNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *InventoryInfoNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *InventoryInfoNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewInventoryInfo(activityID)

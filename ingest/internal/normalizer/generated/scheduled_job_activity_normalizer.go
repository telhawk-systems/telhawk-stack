...
// ScheduledJobActivityNormalizer normalizes scheduled_job_activity events to OCSF format
type ScheduledJobActivityNormalizer struct{}

// NewScheduledJobActivityNormalizer creates a new normalizer
func NewScheduledJobActivityNormalizer() *ScheduledJobActivityNormalizer {
	return &ScheduledJobActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *ScheduledJobActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *ScheduledJobActivityNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := system.NewScheduledJobActivity(activityID)

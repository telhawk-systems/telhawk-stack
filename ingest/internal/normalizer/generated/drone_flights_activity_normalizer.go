...
// DroneFlightsActivityNormalizer normalizes drone_flights_activity events to OCSF format
type DroneFlightsActivityNormalizer struct{}

// NewDroneFlightsActivityNormalizer creates a new normalizer
func NewDroneFlightsActivityNormalizer() *DroneFlightsActivityNormalizer {
	return &DroneFlightsActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *DroneFlightsActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *DroneFlightsActivityNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := unmanned_systems.NewDroneFlightsActivity(activityID)

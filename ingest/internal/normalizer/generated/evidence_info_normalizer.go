...
// EvidenceInfoNormalizer normalizes evidence_info events to OCSF format
type EvidenceInfoNormalizer struct{}

// NewEvidenceInfoNormalizer creates a new normalizer
func NewEvidenceInfoNormalizer() *EvidenceInfoNormalizer {
	return &EvidenceInfoNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *EvidenceInfoNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *EvidenceInfoNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewEvidenceInfo(activityID)

...
// NetworkConnectionQueryNormalizer normalizes network_connection_query events to OCSF format
type NetworkConnectionQueryNormalizer struct{}

// NewNetworkConnectionQueryNormalizer creates a new normalizer
func NewNetworkConnectionQueryNormalizer() *NetworkConnectionQueryNormalizer {
	return &NetworkConnectionQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *NetworkConnectionQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *NetworkConnectionQueryNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewNetworkConnectionQuery(activityID)

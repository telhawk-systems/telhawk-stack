...
// KernelActivityNormalizer normalizes kernel_activity events to OCSF format
type KernelActivityNormalizer struct{}

// NewKernelActivityNormalizer creates a new normalizer
func NewKernelActivityNormalizer() *KernelActivityNormalizer {
	return &KernelActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *KernelActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *KernelActivityNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := system.NewKernelActivity(activityID)

...
// DeviceConfigStateChangeNormalizer normalizes device_config_state_change events to OCSF format
type DeviceConfigStateChangeNormalizer struct{}

// NewDeviceConfigStateChangeNormalizer creates a new normalizer
func NewDeviceConfigStateChangeNormalizer() *DeviceConfigStateChangeNormalizer {
	return &DeviceConfigStateChangeNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *DeviceConfigStateChangeNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
...
func (n *DeviceConfigStateChangeNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewDeviceConfigStateChange(activityID)

...
// GroupManagementNormalizer normalizes group_management events to OCSF format
type GroupManagementNormalizer struct{}

// NewGroupManagementNormalizer creates a new normalizer
func NewGroupManagementNormalizer() *GroupManagementNormalizer {
	return &GroupManagementNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *GroupManagementNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "group_management") || 
	       strings.Contains(st, "group")
}

// Normalize converts raw event to OCSF group_management
func (n *GroupManagementNormalizer) Normalize(ctx context.Context, envelope *models.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := iam.NewGroupManagement(activityID)

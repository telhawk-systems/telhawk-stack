...
// RemediationActivityNormalizer normalizes remediation_activity events to OCSF format
type RemediationActivityNormalizer struct{}

// NewRemediationActivityNormalizer creates a new normalizer
func NewRemediationActivityNormalizer() *RemediationActivityNormalizer {
	return &RemediationActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *RemediationActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "remediation_activity") || 
	       strings.Contains(st, "remediation") || 
	       strings.Contains(st, "remediation")
}

// Normalize converts raw event to OCSF remediation_activity
func (n *RemediationActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := remediation.NewRemediationActivity(activityID)

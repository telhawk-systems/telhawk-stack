...
// FileQueryNormalizer normalizes file_query events to OCSF format
type FileQueryNormalizer struct{}

// NewFileQueryNormalizer creates a new normalizer
func NewFileQueryNormalizer() *FileQueryNormalizer {
	return &FileQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *FileQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "file_query") ||
		strings.Contains(st, "file")
}

// Normalize converts raw event to OCSF file_query
func (n *FileQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewFileQuery(activityID)

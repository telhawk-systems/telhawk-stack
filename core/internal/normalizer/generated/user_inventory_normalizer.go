...
// UserInventoryNormalizer normalizes user_inventory events to OCSF format
type UserInventoryNormalizer struct{}

// NewUserInventoryNormalizer creates a new normalizer
func NewUserInventoryNormalizer() *UserInventoryNormalizer {
	return &UserInventoryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *UserInventoryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "user_inventory") ||
		strings.Contains(st, "user")
}

// Normalize converts raw event to OCSF user_inventory
func (n *UserInventoryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewUserInventory(activityID)

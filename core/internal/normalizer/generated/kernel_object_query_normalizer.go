...
// KernelObjectQueryNormalizer normalizes kernel_object_query events to OCSF format
type KernelObjectQueryNormalizer struct{}

// NewKernelObjectQueryNormalizer creates a new normalizer
func NewKernelObjectQueryNormalizer() *KernelObjectQueryNormalizer {
	return &KernelObjectQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *KernelObjectQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "kernel_object_query") ||
		strings.Contains(st, "kernel")
}

// Normalize converts raw event to OCSF kernel_object_query
func (n *KernelObjectQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewKernelObjectQuery(activityID)

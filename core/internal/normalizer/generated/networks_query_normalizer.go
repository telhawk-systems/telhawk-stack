...
// NetworksQueryNormalizer normalizes networks_query events to OCSF format
type NetworksQueryNormalizer struct{}

// NewNetworksQueryNormalizer creates a new normalizer
func NewNetworksQueryNormalizer() *NetworksQueryNormalizer {
	return &NetworksQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *NetworksQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "networks_query") || 
	       strings.Contains(st, "networks")
}

// Normalize converts raw event to OCSF networks_query
func (n *NetworksQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewNetworksQuery(activityID)

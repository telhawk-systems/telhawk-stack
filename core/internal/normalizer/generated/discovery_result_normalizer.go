...
// DiscoveryResultNormalizer normalizes discovery_result events to OCSF format
type DiscoveryResultNormalizer struct{}

// NewDiscoveryResultNormalizer creates a new normalizer
func NewDiscoveryResultNormalizer() *DiscoveryResultNormalizer {
	return &DiscoveryResultNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *DiscoveryResultNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "discovery_result") || 
	       strings.Contains(st, "discovery")
}

// Normalize converts raw event to OCSF discovery_result
func (n *DiscoveryResultNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewDiscoveryResult(activityID)

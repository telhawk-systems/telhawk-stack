...
// SshActivityNormalizer normalizes ssh_activity events to OCSF format
type SshActivityNormalizer struct{}

// NewSshActivityNormalizer creates a new normalizer
func NewSshActivityNormalizer() *SshActivityNormalizer {
	return &SshActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *SshActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "ssh_activity") ||
		strings.Contains(st, "ssh") ||
		strings.Contains(st, "ssh")
}

// Normalize converts raw event to OCSF ssh_activity
func (n *SshActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := network.NewSshActivity(activityID)

...
// StartupItemQueryNormalizer normalizes startup_item_query events to OCSF format
type StartupItemQueryNormalizer struct{}

// NewStartupItemQueryNormalizer creates a new normalizer
func NewStartupItemQueryNormalizer() *StartupItemQueryNormalizer {
	return &StartupItemQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *StartupItemQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "startup_item_query") || 
	       strings.Contains(st, "startup")
}

// Normalize converts raw event to OCSF startup_item_query
func (n *StartupItemQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewStartupItemQuery(activityID)

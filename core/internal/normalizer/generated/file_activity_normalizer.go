...
// FileActivityNormalizer normalizes file_activity events to OCSF format
type FileActivityNormalizer struct{}

// NewFileActivityNormalizer creates a new normalizer
func NewFileActivityNormalizer() *FileActivityNormalizer {
	return &FileActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *FileActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "file") || 
	       strings.Contains(st, "fmon") || 
	       strings.Contains(st, "fsmon") || 
	       strings.Contains(st, "filesystem") || 
	       strings.Contains(st, "audit")
}

// Normalize converts raw event to OCSF file_activity
func (n *FileActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := system.NewFileActivity(activityID)

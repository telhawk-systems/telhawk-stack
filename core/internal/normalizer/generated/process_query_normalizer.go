...
// ProcessQueryNormalizer normalizes process_query events to OCSF format
type ProcessQueryNormalizer struct{}

// NewProcessQueryNormalizer creates a new normalizer
func NewProcessQueryNormalizer() *ProcessQueryNormalizer {
	return &ProcessQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *ProcessQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "process_query") || 
	       strings.Contains(st, "process")
}

// Normalize converts raw event to OCSF process_query
func (n *ProcessQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewProcessQuery(activityID)

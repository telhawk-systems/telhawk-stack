...
// RdpActivityNormalizer normalizes rdp_activity events to OCSF format
type RdpActivityNormalizer struct{}

// NewRdpActivityNormalizer creates a new normalizer
func NewRdpActivityNormalizer() *RdpActivityNormalizer {
	return &RdpActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *RdpActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "rdp_activity") || 
	       strings.Contains(st, "rdp") || 
	       strings.Contains(st, "rdp")
}

// Normalize converts raw event to OCSF rdp_activity
func (n *RdpActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := network.NewRdpActivity(activityID)

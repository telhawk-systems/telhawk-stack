...
// NtpActivityNormalizer normalizes ntp_activity events to OCSF format
type NtpActivityNormalizer struct{}

// NewNtpActivityNormalizer creates a new normalizer
func NewNtpActivityNormalizer() *NtpActivityNormalizer {
	return &NtpActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *NtpActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "ntp_activity") ||
		strings.Contains(st, "ntp") ||
		strings.Contains(st, "ntp")
}

// Normalize converts raw event to OCSF ntp_activity
func (n *NtpActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := network.NewNtpActivity(activityID)

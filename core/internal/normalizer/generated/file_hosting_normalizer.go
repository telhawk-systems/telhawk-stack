...
// FileHostingNormalizer normalizes file_hosting events to OCSF format
type FileHostingNormalizer struct{}

// NewFileHostingNormalizer creates a new normalizer
func NewFileHostingNormalizer() *FileHostingNormalizer {
	return &FileHostingNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *FileHostingNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "file_hosting") ||
		strings.Contains(st, "file")
}

// Normalize converts raw event to OCSF file_hosting
func (n *FileHostingNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := application.NewFileHosting(activityID)

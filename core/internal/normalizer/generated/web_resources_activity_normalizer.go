...
// WebResourcesActivityNormalizer normalizes web_resources_activity events to OCSF format
type WebResourcesActivityNormalizer struct{}

// NewWebResourcesActivityNormalizer creates a new normalizer
func NewWebResourcesActivityNormalizer() *WebResourcesActivityNormalizer {
	return &WebResourcesActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *WebResourcesActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "web_resources_activity") || 
	       strings.Contains(st, "web") || 
	       strings.Contains(st, "web_resources")
}

// Normalize converts raw event to OCSF web_resources_activity
func (n *WebResourcesActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := application.NewWebResourcesActivity(activityID)

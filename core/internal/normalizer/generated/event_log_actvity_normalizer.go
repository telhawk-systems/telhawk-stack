...
// EventLogActvityNormalizer normalizes event_log_actvity events to OCSF format
type EventLogActvityNormalizer struct{}

// NewEventLogActvityNormalizer creates a new normalizer
func NewEventLogActvityNormalizer() *EventLogActvityNormalizer {
	return &EventLogActvityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *EventLogActvityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "event_log_actvity") ||
		strings.Contains(st, "event")
}

// Normalize converts raw event to OCSF event_log_actvity
func (n *EventLogActvityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := system.NewEventLogActvity(activityID)

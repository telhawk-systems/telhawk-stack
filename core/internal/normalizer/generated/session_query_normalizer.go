...
// SessionQueryNormalizer normalizes session_query events to OCSF format
type SessionQueryNormalizer struct{}

// NewSessionQueryNormalizer creates a new normalizer
func NewSessionQueryNormalizer() *SessionQueryNormalizer {
	return &SessionQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *SessionQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "session_query") || 
	       strings.Contains(st, "session")
}

// Normalize converts raw event to OCSF session_query
func (n *SessionQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewSessionQuery(activityID)

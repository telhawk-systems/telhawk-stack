...
// EmailActivityNormalizer normalizes email_activity events to OCSF format
type EmailActivityNormalizer struct{}

// NewEmailActivityNormalizer creates a new normalizer
func NewEmailActivityNormalizer() *EmailActivityNormalizer {
	return &EmailActivityNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *EmailActivityNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "email_activity") ||
		strings.Contains(st, "email") ||
		strings.Contains(st, "email")
}

// Normalize converts raw event to OCSF email_activity
func (n *EmailActivityNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := network.NewEmailActivity(activityID)

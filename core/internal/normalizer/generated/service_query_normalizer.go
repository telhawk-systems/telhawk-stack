...
// ServiceQueryNormalizer normalizes service_query events to OCSF format
type ServiceQueryNormalizer struct{}

// NewServiceQueryNormalizer creates a new normalizer
func NewServiceQueryNormalizer() *ServiceQueryNormalizer {
	return &ServiceQueryNormalizer{}
}

// Supports checks if this normalizer handles the given event
func (n *ServiceQueryNormalizer) Supports(format, sourceType string) bool {
	if format != "json" {
		return false
	}
	st := strings.ToLower(sourceType)
	return strings.Contains(st, "service_query") ||
		strings.Contains(st, "service")
}

// Normalize converts raw event to OCSF service_query
func (n *ServiceQueryNormalizer) Normalize(ctx context.Context, envelope *model.RawEventEnvelope) (*ocsf.Event, error) {
	...
	event := discovery.NewServiceQuery(activityID)
